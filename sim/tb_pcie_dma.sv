...
`ifdef USE_AHB
    // F3) MID-BURST SYS bus error (AHB only): inject the two-cycle ERROR on a
...
`endif
    ...
    // I) unaligned descriptor ring base (bus-independent): rejected at GO with
    ...
    // J) graceful stop (issue #14): CTRL.STOP lets the descriptor in flight
